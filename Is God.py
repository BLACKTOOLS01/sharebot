...
ADMIN = 1954081824 # TUO CHAT ID

API_KEY = 7086869 #api id
API_HASH = "fec483a1274d1fa5204f964d745f765d"
STRING_SESSION = ""
ADMINS = []
Getter = None
Number = None
    ...
    if e.is_private:
        if e.chat_id == ADMIN or e.chat_id in ADMINS:
            if e.text == "/start":
                ...
                if archivialimitati:
                    if raspacontrol():
                        await e.respond("**DEI VOIP SONO STATI RI-AGGIUNTI\n🤖 Pannello Raspa Bot\n\n⚙ Versione » 4.4**",
...
async def callbackQuery(e):
    ...
    if e.sender_id == ADMIN or e.sender_id in ADMINS:
        if e.data == b"back":
            Getter, Number, TempClient = None, None, None
            await e.edit("**🤖 Pannello Raspa Bot\n\n⚙ Versione » 4.4**", buttons=[[Button.inline("📞 Voip", "voip")],
            ...
            if st[0] == "setnome":
                ...
                if st[1] in SSs:
                    await e.edit(
                        ...
                        buttons=[
                            [Button.inline("🔶OTTIENI MESSAGGIO🔶", "getmsg;" + st[1])], [Button.inline("🔸SETTA USERNAME🔸", "setusername;" + st[1])],
